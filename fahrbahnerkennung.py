...
def nur_relevante_Region(edges):
    #problem: eckenerkennung() auch viele in oberer hälfte

    #params intialisieren
    height, width = edges.shape
    mask = np.zeros_like(edges)

    #nur untere Hälfte ist interessant
    polygon = np.array([[
        (0, height * 1 / 2),
        (width, height * 1 / 2),
        (width, height),
...
def main():
    bild = cv2.imread('/home/pi/Test/test2.jpg')
    #fahrbahn?
...
class HandCodedLaneFollower(object):
    ...
    def __init__(self, car=None):
        ...
        self.car = car
        self.curr_steering_angle = 90 #default

    def folge_Fahrbahn(self, frame):
        # Main entry point of the lane follower
        show_image("Original", frame)

        fahrbahn= erkenne_Fahrbahn(frame)
        finales_bild = self.steuer(frame, lane_lines)
    ...
    def steuer(self, bild, fahrbahn):
        ...
        curr_heading_image = display_heading_line(bild, self.m_steuer_winkel)
